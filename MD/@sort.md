...
本次更新重构了 `onGlobalMouseMove` 中的排序判别算法，实现了上述提到的关键优化，重点提升了操作的稳定性和流畅度。

## 1. 核心流程优化

### 智能屏蔽“自身” (Ignore Self)
*   **实现**: 在计算排序位置前，优先检查目标行对应的歌曲 (`targetSong`) 是否存在于当前正在拖拽的歌曲集合 (`dragSession.songs`) 中。
*   **效果**: 如果是拖拽自身，立即隐藏红线 (`sortLineTop = -1`) 并停止后续计算。这避免了用户将歌曲拖到自己身上时出现的无效视觉提示。

### 引入“死区”与滞后性 (Hysteresis)
*   **实现**: 引入了状态记忆变量 `lastTargetRowPath` 和 `lastInsertPosition`。
    *   **新行判定**: 当鼠标首次进入一行时，使用标准的 **50%** 阈值进行初步判定。
    *   **行内微调**: 当鼠标在同一行内移动时：
        *   **< 30%**: 强制判定为“上半区” (Insert Before)。
        *   **> 70%**: 强制判定为“下半区” (Insert After)。
        *   **30% - 70%**: **保持上一次的判定结果不变**。
*   **效果**: 彻底解决了在行中间位置微小手抖导致红线上下跳动的问题，提供了“粘性”的稳定手感。

### 边缘自动滚动 (Auto Scroll)
*   **实现**:
    *   通过 `songTableRef` 直接获取歌曲列表容器的 DOM 引用。
    *   监听鼠标 Y 坐标：
        *   距离容器顶部 **< 60px**: 触发向上滚动。
        *   距离容器底部 **< 60px**: 触发向下滚动。
    *   使用 `requestAnimationFrame` 实现平滑滚动循环。
*   **效果**: 允许用户轻松将歌曲拖拽到长列表的任意位置，无需手动滚动。

## 2. 逻辑流向图 (更新后)

```mermaid
graph TD
    A[Global MouseMove] --> B{Drag Active?}
    B -- No --> C[Ignore]
    B -- Yes --> D[Auto Scroll Check]
    D --> E[Identify Target]
    E -- Sidebar --> F[Folder/Playlist Logic]
    E -- Table Row --> G{Target is Self?}
    G -- Yes --> H[Hide Sort Line]
    G -- No --> I[Ratio Calculation]
    I --> J{Same Row as Last?}
    J -- No --> K[Threshold 50%]
    J -- Yes --> L{Hysteresis Check}
    L -- < 30% --> M[Insert Before]
    L -- > 70% --> N[Insert After]
    L -- 30-70% --> O[Keep Last State]
    M --> P[Update InsertIndex]
    N --> P
    K --> P
    O --> P
```

## 3. 关键代码变更 (`SongList.vue`)

```typescript
// 伪代码展示核心逻辑
if (lastTargetRowPath === targetSong?.path) {
  if (ratio < 0.3) {
    insertBefore = true;
  } else if (ratio > 0.7) {
    insertBefore = false;
  } else {
    insertBefore = (lastInsertPosition === 'before'); // 保持状态
  }
} else {
  insertBefore = ratio < 0.5; // 新行默认
}
```

这些改进使得排序功能达到了专业级应用的交互标准。
